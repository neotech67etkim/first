...
| `NAVIS_AUTO_INITIAL_WAIT_SECONDS` | 아니오 | 로딩 대화상자가 사라진 뒤 **무조건** 기다리는 시간(초). 기본 60초 |
...
3. 문서가 열린 게 확인되면(`Models.Count > 0`) 어떤 창을 대상으로 캡처할지
   ...
   **로딩 대화상자가 사라진 뒤 구간은 반응형 감지를 포기하고
   ...
   이제 "무조건 이만큼 기다림"입니다). 관측점 하나하나를 캡처할 때는
...
```
L60_D42_S60_MyViewpointName_stable1.8s.png
└┬─┘└┬─┘└┬─┘└──────┬──────┘└─────┬─────┘
 │   │   │         │              └ 관측점 캡처: 1.8초 만에 안정됨
 │   │   │         └ 관측점 이름
 │   │   └ 로딩 대화상자 사라진 뒤 무조건 대기(초) — 이 실행에서 실제로 60초
 │   └ 로딩 대화상자가 보였던 시간(초). 대화상자를 아예 못 봤으면 `Dnone`
 └ 로딩 대화상자 뜨기 전 무조건 대기(초) — 이 실행에서 실제로 60초
```
