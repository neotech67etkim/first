...
3. 문서가 열린 게 확인되면(`Models.Count > 0`) 어떤 창을 대상으로 캡처할지
   ...
   **`NAVIS_AUTO_MIN_INITIAL_WAIT_SECONDS`만큼 조건 없이 무조건
   ...
   "화면이 안정될 때까지" 방식**을 씁니다: 화면을 반복 캡처해서(실제
   크기 그대로는 느리니 32×32로 축소한 축약본끼리) 직전 캡처와 비교하고,
   **연속으로 여러 번 계속 똑같아야만** "안정됐다"고 보고 그 시점의
   캡처를 사용합니다. 처음엔 "연속 두 번만 같으면 통과"였는데, 대용량
   모델은 지오메트리가 뭉텅이로 스트리밍되면서 중간중간 짧게 멈추는
   구간이 있어서 그 멈춤을 로딩 완료로 오판해 실제로 덜 로딩된 상태가
   캡처되는 문제가 있었습니다. 그래서 로딩 후 대기는 **1초 간격으로
   6번 연속(즉 최소 6초 동안 계속 무변화)**, 관측점 캡처 대기는 **0.5초
   간격으로 4번 연속(최소 2초)**을 요구하도록 강화했습니다 — 순간적인
   멈춤보다 훨씬 긴 구간이라 오판할 가능성이 낮습니다.
   (`NAVIS_AUTO_INITIAL_WAIT_SECONDS`/`NAVIS_AUTO_WAIT_SECONDS`는 각각 이
   대기의 **최대 상한**이고, 그 안에 안정되지 않으면 포기하고 마지막
   캡처를 그냥 씁니다 — 로딩 후 대기 쪽은 이 강화된 기준을 만족할 여유를
   주려고 기본값을 60초로 늘렸습니다.) 비교는 완전히 동일한 픽셀이 아니라
   오차 허용치를 두어서, GPU/안티앨리어싱 노이즈로 생기는 미세한 픽셀
   차이 때문에 계속 "안 안정됨"으로 오판하지 않도록 했습니다. 이 폴링
   루프 안에서도 매 반복마다 창을 한 번 더 맨 앞으로
