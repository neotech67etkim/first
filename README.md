...
| `NAVIS_AUTO_WAIT_SECONDS` | 아니오 | 관측점 이동 후 캡처까지 대기 시간(초). 기본 15초 |
| `NAVIS_AUTO_INITIAL_WAIT_SECONDS` | 아니오 | 문서가 열린 뒤 캡처 루프 시작 전 대기 시간(초). 기본 15초 |
...
3. 문서가 열린 게 확인되면(`Models.Count > 0`) Win32 `SetForegroundWindow`
   (+ 최소화 상태면 `ShowWindow`로 복원)로 Navisworks 창을 **맨 앞으로
   가져옵니다.** 캡처는 오프스크린 렌더링이 아니라 화면에 실제로 보이는
   내용을 그대로 찍는 방식(`Graphics.CopyFromScreen`)이라, 창이 다른
   창에 가려져 있으면 엉뚱한 화면(다른 모니터/다른 창)이 찍히기
   때문입니다. 이후 그림이 다 로딩될 시간을 주기 위해
   **`NAVIS_AUTO_INITIAL_WAIT_SECONDS`만큼 한 번 더 대기**한 뒤 저장된
   관측점을 순회합니다. 관측점마다는 사람이 누르는 **캡처** 버튼 대신
   **관측점 이동 후 `NAVIS_AUTO_WAIT_SECONDS`만큼 고정 대기**했다가,
   대기 중 다른 창(알림 팝업 등)이 포커스를 가져갔을 수 있으니 캡처
   직전에 창을 한 번 더 맨 앞으로 가져온 뒤 캡처합니다. (대화상자를
