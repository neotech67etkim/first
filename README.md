...
3. 문서가 열린 게 확인되면(`Models.Count > 0`) 곧바로 캡처를 시작하지
   않고, 그림이 다 로딩될 시간을 주기 위해 **`NAVIS_AUTO_INITIAL_WAIT_SECONDS`
   만큼 한 번 더 대기**한 뒤 저장된 관측점을 순회합니다. 관측점마다는
   사람이 누르는 **캡처** 버튼 대신 **관측점 이동 후
   `NAVIS_AUTO_WAIT_SECONDS`만큼 고정 대기**했다가 캡처합니다.
